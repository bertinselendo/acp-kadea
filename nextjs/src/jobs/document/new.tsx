...
client.defineJob({
  id: "new-document-notification",
  name: "New Document Notification",
  version: "0.1.0",
  trigger: eventTrigger({
    name: "send.new-document-notification",
    schema: z.object({
...
export const sendNewDocumentNotification = async ({
  ...
  const heading = `New document - ${reference}`;
  const body = `A new document has been added to the project ${reference} by ${senderName}.`;
  const footer = "";
